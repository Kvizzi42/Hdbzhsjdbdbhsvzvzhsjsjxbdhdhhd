...
async def check_notifies():
    ...
    notifies = await db.select([Money.amount, Money.percent, Money.uid_worker, Money.trans, MoneyNotifies.message_id, Money.payed,
        ...
        .where(MoneyNotifies.edited == False).where(Money.country == 'pl').gino.all()
    for n in notifies:
        print(n.uid_worker)
        try:
            if n.payed:
                ...
                try:
                    ...
                    text = '🇵🇱 Польша\n' \
                           ...
                           '💵Статус: [Оплачено]💵\n' \
                           '📊Процент работника: {}%\n' \
                           '{}'.format(get_rub(n.amount), n.amount, round(percent, 0),
                                                        user_text)
                ...
                try:
                    await bot.send_message(worker.uid, '✅Сумма: {}✅ \n💵 Статус: [Оплачено]💵\n'
                                                       '<b>Внимание:</b> курс PLN к рублю является условным '
                ...
                if not is_chat_member:
                    money = await Money.query.where(Money.uid_worker == worker.uid).where(Money.country == 'pl').gino.all()
                    total_sum = 0
                    for m in money:
                        total_sum += m.amount
                    if total_sum >= 5000:
                        await bot.send_message(worker.uid,
                                               '🎉Ура! Ты внес более 5000 PLN🤑\n'
                        ...
                        await bot.send_message(main_config.ERROR_CHANNEL, 'PL\nНовый воркер в про чат: @{}\n'
                                                                     'Занес: {} PLN'.format(worker.username, total_sum))
                    else:
                        await bot.send_message(main_config.ERROR_CHANNEL, 'PL \nВоркер: @{}\n'
                                                                     'Всего наворкал: {} PLN\n'
                                                                     'До про чата осталось: {} PLN'
...
def get_rub(PLN):
    return round(PLN * 18.5, 0)


async def is_lvl2_member(uid, bot: Bot):
    try:
        member = await bot.get_chat_member(main_config.PL_LVL2_CHAT, uid)
        if member:
