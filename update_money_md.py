...
async def check_notifies():
    ...
    notifies = await db.select([Money.amount, Money.percent, Money.uid_worker, MoneyNotifies.message_id, Money.payed,
        ...
        .where(MoneyNotifies.edited == False).where(Money.country == 'md').gino.all()
    for n in notifies:
        print(n.uid_worker)
        try:
            if n.payed:
                ...
                try:
                    await bot.send_message(worker.uid, '✅Сумма: {}✅ \n💵 Статус: [Оплачено]💵\n'
                                                       '<b>Внимание:</b> курс MDL к рублю является условным '
                ...
                if not is_chat_member:
                    money = await Money.query.where(Money.uid_worker == worker.uid).where(Money.country == 'md').gino.all()
                    total_sum = 0
                    for m in money:
                        total_sum += m.amount
                    if total_sum >= 7500:
                        await bot.send_message(worker.uid,
                                               '🎉Ура! Ты внес более 7500 MDL🤑\n'
                                               '😱Теперь ты получаешь доступ в PRO-чат\n'
                                               'Для вступления отпиши сюда 👉🏻 @onegin_02')
                        await bot.send_message(main_config.ERROR_CHANNEL, 'MD\nНовый воркер в про чат: @{}\n'
                                                                     'Нотиф воркеру пришел'.format(worker.username))
                    else:
                        await bot.send_message(main_config.ERROR_CHANNEL, 'MD \nВоркер: @{}\n'
                                                                     'Всего наворкал: {} MDL\n'
                                                                     'До про чата осталось: {} MDL'
                                               .format(worker.username, total_sum, (7500 - total_sum)))
