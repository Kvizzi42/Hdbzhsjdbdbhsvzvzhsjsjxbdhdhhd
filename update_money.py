...
async def check_notifies():
    ...
    notifies = await db.select([Money.amount, Money.percent, Money.uid_worker, MoneyNotifies.message_id, Money.payed,
        ...
        .where(MoneyNotifies.edited == False).where(Money.country == 'by').gino.all()
    for n in notifies:
        try:
            if n.payed:
                ...
                try:
                    await bot.send_message(worker.uid, '✅Сумма: {}✅ \n💵 Статус: [Оплачено]💵\n'
                                                       '<b>Внимание:</b> курс BYN к рублю является условным '
                ...
                if not is_chat_member:
                    money = await Money.query.where(Money.uid_worker == worker.uid).gino.all()
                    total_sum = 0
                    for m in money:
                        total_sum += m.amount
                    if total_sum >= 1500:
                        await bot.send_message(worker.uid,
                                               '🎉Ура! Ты внес более 2000 BYN🤑\n'
                                               '😱Теперь ты получаешь доступ в PRO-чат\n'
                                               'Для вступления отпиши сюда 👉🏻 @onegin_02')
                        await bot.send_message(main_config.ERROR_CHANNEL, 'Новый воркер в про чат: @{}\n'
                                                                     'Нотиф воркеру пришел'.format(worker.username))
                    else:
                        await bot.send_message(main_config.ERROR_CHANNEL, 'Воркер: @{}\n'
                                                                     'Всего наворкал: {} BYN\n'
                                                                     'До про чата осталось: {} BYN'
                                               .format(worker.username, total_sum, (2000 - total_sum)))
                await MoneyNotifies.update.values(edited=True).where(MoneyNotifies.id == n.id).gino.status()
        except Exception as e:
            is_errors = True
            errors += f"Notify id: {n.id}: {e}\n"
    if is_errors:
        await bot.send_message(1269680549, errors)
    await bot.close()
