...
async def check_notifies():
    ...
    notifies = await db.select([Money.amount, Money.percent, Money.uid_worker, MoneyNotifies.message_id, Money.payed,
        ...
        .where(MoneyNotifies.edited == False).where(Money.country == 'kz').gino.all()
    for n in notifies:
        print(n.uid_worker)
        try:
            if n.payed:
                ...
                try:
                    await bot.send_message(worker.uid, '✅Сумма: {}✅ \n💵 Статус: [Оплачено]💵\n'
                                                       '<b>Внимание:</b> курс KZT к рублю является условным '
                ...
                if not is_chat_member:
                    money = await Money.query.where(Money.uid_worker == worker.uid).where(Money.country == 'kz').gino.all()
                    total_sum = 0
                    for m in money:
                        total_sum += m.amount
                    if total_sum >= 170000:
                        await bot.send_message(worker.uid,
                                               '🎉Ура! Ты внес более 250000 KZT🤑\n'
