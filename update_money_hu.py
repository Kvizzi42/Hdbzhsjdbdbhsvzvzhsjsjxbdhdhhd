...
async def check_notifies():
    ...
    notifies = await db.select([Money.amount, Money.percent, Money.uid_worker, MoneyNotifies.message_id, Money.payed,
        ...
        .where(MoneyNotifies.edited == False).where(Money.country == 'hu').gino.all()
    for n in notifies:
        print(n.uid_worker)
        try:
            if n.payed:
                ...
                try:
                    await bot.send_message(worker.uid, '✅Сумма: {}✅ \n💵 Статус: [Оплачено]💵\n'
                                                       '<b>Внимание:</b> курс HUF к рублю является условным '
                ...
                if not is_chat_member:
                    money = await Money.query.where(Money.uid_worker == worker.uid).where(Money.country == 'hu').gino.all()
                    total_sum = 0
                    for m in money:
                        total_sum += m.amount
                    if total_sum >= 1700000:
                        await bot.send_message(worker.uid,
                                               '🎉Ура! Ты внес более 250000 HUF🤑\n'
                        ...
                        await bot.send_message(main_config.ERROR_CHANNEL, 'HU \nВоркер: @{}\n'
                                                                     'Всего наворкал: {} HUF\n'
                                                                     'До про чата осталось: {} HUF'
