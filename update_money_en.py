...
async def check_notifies():
    ...
    notifies = await db.select([Money.amount, Money.percent, Money.uid_worker, MoneyNotifies.message_id, Money.payed,
        ...
        .where(MoneyNotifies.edited == False).where(Money.country == 'en').gino.all()
    for n in notifies:
        print(n.uid_worker)
        try:
            if n.payed:
                ...
                if not is_chat_member:
                    money = await Money.query.where(Money.uid_worker == worker.uid).where(Money.country == 'en').gino.all()
                    total_sum = 0
                    for m in money:
                        total_sum += m.amount
                    if total_sum >= 5000:
                        await bot.send_message(worker.uid,
                                               '🎉Ура! Ты внес более 5000 GBP🤑\n'
                                               '😱Теперь ты получаешь доступ в PRO-чат\n'
                                               'Для вступления отпиши сюда 👉🏻 @onegin_02')
                        await bot.send_message(main_config.ERROR_CHANNEL, 'EN\nНовый воркер в про чат: @{}\n'
